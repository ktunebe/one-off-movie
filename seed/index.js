...
mongooseConnection.once('connected', async () => {
  ...
  for (const thoughtSeed of thoughtSeeds) {
    thoughtSeed.username = userSeeds[Math.floor(Math.random() * userSeeds.length)].username
    let reactionArray = []
    for (const reaction of reactionSeeds) {
      reaction.username = userSeeds[Math.floor(Math.random() * userSeeds.length)].username
      if (.3 > Math.random()) {
